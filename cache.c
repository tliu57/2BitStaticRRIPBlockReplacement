...
/* select a victim with saturated RRPV */
struct cache_blk_t *srrip_victim_selection(struct cache_set_t *set)
{
	struct cache_blk_t* blk;
	while(1){
		for (blk = set->way_head; blk; blk = blk->way_next){
			...
			if (blk->RRPV == 3){
				/* first '3' found, set RRPV as 2 for the incoming introduced block */
				blk->RRPV = 2;
				return blk;
			}
		}
		...
		for (blk = set->way_head; blk; blk = blk->way_next){
			//if(blk->RRPV < 3)
			// no '3' found, increment all RRPVs
			blk->RRPV++;
		}
	}
}
		...
		md_addr_t *repl_addr)	/* for address of replaced block */
{
	...
	switch (cp->policy) {
		...
		case SRRIP:
			//search code
			repl = srrip_victim_selection(&cp->sets[set]);
			// update_way_list(&cp->sets[set], repl, Head);
...
cache_hit: /* slow hit handler */
	...
	// if SRRIP replacement, decrease the PPRV of the hit block according to SRRIP-FP policy
	if(cp->policy == SRRIP && blk->RRPV > 0)
		blk->RRPV--;
...
cache_fast_hit: /* fast hit handler */
	...
	//if SRRIP replacement, decrease the PPRV of the hitten block using SRRIP-FP policy
	if(cp->policy == SRRIP && blk->RRPV > 0)
		blk->RRPV--;
		...
		tick_t now)			/* time of cache flush */
{
	...
	for (i=0; i<cp->nsets; i++)
	{
		for (blk=cp->sets[i].way_head; blk; blk=blk->way_next)
		{
			...
			// set RRPV to 3 when invalidating the cache block
			blk->RRPV = 3;
		}
	}
